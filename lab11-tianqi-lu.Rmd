...
```{r echo = FALSE}
dat <- read.csv('~/downloads/stat133/stat133-hws-fall17/mobile-food-sf.csv', stringsAsFactors = FALSE)
```
...
# subset time
str_sub(times[1: 2], start = 1, end = 2)
str_sub(times[3: 4], start = 1, end = 1)
# subset period
str_sub(times[1: 2], start = 3, end = 4)
str_sub(times[3: 4], start = 2, end = 3)
...
for (i in 1:length(hours)) {
  if (periods[i] == "AM" | (periods[i] == "PM" & hours[i] == 12)) {
    start24[i] <- paste(hours[i], ":00", sep = "") 
  } else {
    start24[i] <- paste(hours[i] + 12, ":00", sep = "")
  }
}
  start24
}
...
```{r}
# add 'start' hours
findHours <- function(times) {
  as.numeric(str_replace(times, pattern = 'AM|PM', replacement = ''))
}
findPeriods <- function(times) {
  str_sub(times, start = -2)
}
a <- to24(findHours(dat$starttime), findPeriods(dat$starttime))
b <- str_replace(a, pattern = ':00', replacement = '')
dat$start <- as.numeric(b)

# add 'end' hours
aa <- to24(findHours(dat$endtime), findPeriods(dat$endtime))
bb <- str_replace(aa, pattern = ':00', replacement = '')
dat$end <- as.numeric(bb)
```
