...
```{r}
head(select(filter(dat, experience == 0 & age == 20), player, team, height, weight), 5)
```
...
```{r}
gsw2 <- mutate(dat, min_per_game = minutes / games)
gsw_mpg <- select(arrange(gsw2, desc(min_per_game)), player, experience, min_per_game)
gsw_mpg
```
...
```{r}
ggplot(filter(dat, position == "C"), aes(x = experience, y = salary)) + geom_smooth(method = "lm") + geom_point()
```
...
```{r}
ggplot(dat, aes(dat$age)) + geom_density() + facet_wrap(~ team)
```
...
```{r}
ggplot(dat, aes(x = height, y = weight)) + geom_point() + geom_density2d() + facet_wrap(~ position)
```
...
```{r}
ggplot(dat, aes(x = height, y = weight)) + geom_point() + geom_density2d() + facet_wrap(~ position) + theme_dark()
```