...
```{r}
# frequency table
freqTable <- data.frame(states, num_chars)
freqTable
# barchart of number-of-characters
ggplot(freqTable, aes(x = num_chars)) + geom_bar()
...
for(i in min(num_chars) : max(num_chars)) {
  if (i %% 2 == 0) {
    states_list[[i - min(num_chars) + 1]] <- toupper(paste(states[num_chars == i], collapse = ", "))
  } else {
    states_list[[i - min(num_chars) + 1]] <- tolower(paste(states[num_chars == i], collapse = ", "))
  }
}
...
gsub(a, pattern="file", replacement="dataset")
...
cat("title: \"Some title\"", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("author: \"Your name\"", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("date: \"today's date\"", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("output: html_document", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("---", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("This is the first line", file = outfile2)
cat("\n", file = outfile2, append = TRUE)
cat("A 2nd line", file = outfile2, append = TRUE)
cat("\n\n", file = outfile2, append = TRUE)
cat("\nThe quick brown fox jumps over the lazy dog\n",
    file = outfile2, append = TRUE)
```
...
```{r eval = FALSE}
colplot <- function(caler) {
  if (!is_color(caler)) {
    stop(paste('invalid color', caler))
  } else {
    x <- rnorm(50)
    y <- rnorm(50)
    plot(x, y, col = caler, pch = 20)
    title(paste('Testing color', caler))
  }
}
# this should plot
colplot('tomato')
